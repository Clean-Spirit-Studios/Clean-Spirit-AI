...
- Download the **CPU AAR** (`llama-cpp-dart.aar`), not the Hexagon/NPU one.
  - We tried the Hexagon AAR (`llama-cpp-dart-hexagon.aar`) for GPU/OpenCL
    offload on a real Snapdragon phone (Nord CE 4) and it failed at
    runtime: `dlopen failed: library "libOpenCL.so" not found`. This
    isn't project-specific — most retail Android phones don't expose
    their GPU vendor's OpenCL driver to third-party apps at all (Android
    restricts loading vendor-partition libraries unless the OEM
    specifically whitelists it). There's no reliable app-side fix, so
    this project sticks with CPU-only on Android. Feel free to
    experiment with the Hexagon AAR on your own device, but don't expect
...
- Make sure `minSdk` is at least 24 (26 as currently configured).
...
- **Speed**: a few tokens/sec on a mid-range Android phone (CPU-only —
  see the Hexagon AAR note above for why GPU offload isn't reliably
  available on Android), faster on recent iPhones (Metal acceleration
  via `gpuLayers: 99` on iOS/macOS).
